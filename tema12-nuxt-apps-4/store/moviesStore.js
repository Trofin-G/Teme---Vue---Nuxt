export const state = () => ({
  moviesList: [
    ...
    },
    {
      ...
      Poster: "https://m.media-amazon.com/images/I/91Xmg5sQQZL.jpg",
      Metascore: "65",
      imdbRating: "7.2",
      imdbVotes: "533,874",
      imdbID: "tt0480249",
      Type: "movie",
      Response: "True",
      shadowBack: "shadowEffectTwo",
      Images: [
        "https://wallpapercave.com/wp/9CT8Jdx.jpg",
        "https://images2.alphacoders.com/674/674470.jpg",
        "https://wallpapercave.com/wp/L4QRyWz.jpg",
...
export const getters = {
  ...
  getAllSortingTypes(state) {
    return state.allSortingTypes;
  },

  getFilterBy(state) {
    return state.resultsMoviesList;
  },
...
export const mutations = {
  setMoviesList: (state, movie) => (state.moviesList = movie),

  setAllYears(state) {
    state.moviesList.forEach((movie) => {
      state.allYears.push(movie.Year.substr(0, 4));
    });

    state.allYears = state.allYears
      .filter((item, index) => state.allYears.indexOf(item) === index)
      .sort()
      .reverse();
  },
  ...
  setAllGenres(state) {
    state.moviesList.forEach((movie) => {
      state.allGenres.push([movie.Genre]);
    });

    state.allGenres = state.allGenres
      .map((item) => item[0].replaceAll(" ", ""))
      .join()
      .split(",");

    state.allGenres = state.allGenres
      .filter((item, index) => state.allGenres.indexOf(item) === index)
      .sort();
  },

  setFilterBy(state, filterByValue) {
    let checkIfSomeAreNull;
    let checkIfEveryAreNull;

    if (filterByValue) {
      checkIfSomeAreNull = filterByValue.some((item) => item == null);
      checkIfEveryAreNull = filterByValue.every((item) => item == null);
    }

    if (filterByValue && !checkIfEveryAreNull) {
      state.resultsMoviesList = state.moviesList.filter((movie) => {
        return checkIfSomeAreNull
          ? movie.Released.includes(filterByValue[0]) ||
              movie.Genre.includes(filterByValue[1])
          : movie.Released.includes(filterByValue[0]) &&
              movie.Genre.includes(filterByValue[1]);
      });
    } else {
      state.resultsMoviesList = state.moviesList;
    }
  },

  setSortBy(state, sortByValue) {
    state.resultsMoviesList = state.resultsMoviesList.sort((a, b) => {
      if (sortByValue == "Year ↓") {
        return a.Year.substr(0, 4).localeCompare(b.Year.substr(0, 4));
      } else if (sortByValue == "Year ↑") {
        return b.Year.substr(0, 4).localeCompare(a.Year.substr(0, 4));
      } else if (sortByValue == "Duration ↓") {
        return a.Runtime.slice(0, -4) - b.Runtime.slice(0, -4);
      } else if (sortByValue == "Duration ↑") {
        return b.Runtime.slice(0, -4) - a.Runtime.slice(0, -4);
  ...
  setSinglePage(state, valueSlug) {
    state.moviesList.map((item) => {
      if (String(item.id) == String(valueSlug)) {
        return (state.resultsMoviesList = item);
      }
    });

    console.log(state.resultsMoviesList, "state.resultsMoviesList");
  },
};

export const actions = {
  sendFilterByValue({ commit, dispatch, state }, filterByValue) {
    commit("setFilterBy", filterByValue);
  },
  ...
  sendValueSlug({ commit, dispatch, state }, valueSlug) {
    commit("setSinglePage", `${valueSlug}`);
  },
};
